    ...
    def social_media_analyst_node(state):
        with analyst_llm_phase("social"):
            ...
            system_message = (
                ...
                + " Use `get_news(ticker, start_date, end_date)` for company-focused corpus (**①②④⑤⑥⑦**, including stock/industry research reports). You may also call `get_global_news(curr_date, look_back_days, limit)` for global macro context (**③ npr + ⑧ macro vector topic**) when it helps explain sentiment or narrative shifts. For ④⑤, read the merged Qdrant corpus directly (no LLM-screened high/medium labels). Try to look at all sources possible from social media to sentiment to news. Provide specific, actionable insights with supporting evidence to help traders make informed decisions."
