    ...
    def news_analyst_node(state):
        with analyst_llm_phase("news"):
            ...
            system_message = (
                ...
                "`get_industry_peers(ticker, curr_date, max_peers)` to list model-selected listed A-share competitors. "
                + get_industry_peer_instruction()
                + " "
                ...
                + " **Tool split (important):** `get_global_news` now only provides **⑧ macro vector topic**; `get_news` provides the remaining company-focused sections (**①②④⑤⑦**, including stock/industry research reports). For ④⑤, use the merged news corpus returned by `get_news` directly (no LLM screening labels such as high/medium)."
