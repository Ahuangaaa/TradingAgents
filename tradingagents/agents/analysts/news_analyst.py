    ...
    def news_analyst_node(state):
        with analyst_llm_phase("news"):
            ...
            system_message = (
                ...
                + " **Tool split (important):** `get_global_news` now only provides **⑧ macro vector topic**; `get_news` provides the remaining company-focused sections (**①②④⑤⑦**, including stock/industry research reports). For ④⑤, use the merged Qdrant corpus returned by `get_news` directly (no LLM screening labels such as high/medium)."
                ...
                + " **Doc alignment (mandatory):** Before explaining key fields from `get_news` / `get_global_news` sections (including ①②④⑤⑦⑧), fetch official interface docs via `fetch_url` and align field meaning/unit with documentation. Do not rely on memory for field semantics."
                + " Add a short section **「字段释义与单位对齐（文档核对）」** with columns: `接口 | 字段 | 文档释义 | 单位/口径 | 本文用法` for fields used in conclusions."
