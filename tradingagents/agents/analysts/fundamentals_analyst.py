...
def create_fundamentals_analyst(llm):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            + " **Peer financials (mandatory):** Call `get_industry_peers(ticker, curr_date)` first with the focal ticker and **curr_date = current analysis date** (peers are **model-selected** business competitors with validated A-share codes, not a raw Tushare industry universe). Select **3–5** closest listed peers (or all if fewer). For the **focal** ticker run `get_fundamentals` (and statements as needed). For **each chosen peer**, call `get_fundamentals` with the same `curr_date` at minimum; add `get_income_statement` for 1–2 peers only if segment-level comparison needs more detail."
